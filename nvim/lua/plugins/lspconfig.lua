...
return {
	...
	config = function()
		...
		local client_format_overrides = {
			eslint = function()
				vim.cmd("silent! EslintFixAll")
			end,
			-- If removed, only enable the default formatter if necessary (performance)
			tsserver = function()
		...
		vim.api.nvim_create_autocmd("LspAttach", {
			...
			callback = function(args)
				...
				vim.api.nvim_create_autocmd("BufWritePre", {
					group = vim.api.nvim_create_augroup("lsp-auto-format", { clear = true }),
					callback = function()
						...
						local default_formatting_capable_client_ids = {}
						for _, client in ipairs(clients) do
							if client_format_overrides[client.name] then
								client_format_overrides[client.name]()
							else
								if
								...
								then
									table.insert(default_formatting_capable_client_ids, client.id)
								end
							end
						end

						if #default_formatting_capable_client_ids == 0 then
							return
						end
						...
						vim.lsp.buf.format({
							...
							filter = function(client)
								return vim.tbl_contains(default_formatting_capable_client_ids, client.id)
							end,
						})
					end,
				})
